    ...
    ACCEPT_LABEL:
        ...
        while(1)
        {
            printf("waiting for client...\n");
             n=recv(tcp_str->cli_sockfd,recBuf,0x1000,MSG_NOSIGNAL);
             	  if(n==-1){
             		  printf("recv error\n");
                      //usleep(5000000);
             		  break;
             ...
             tcp_pack.device_info=(car_device_info*)(recBuf+headerSize);
             memcpy(&tcp_pack,recBuf,headerSize);
             printf("pack size %d,device num %d\n",n,tcp_pack.device_num);
     ...
     LABEL_ACCEP:   while(1){
        ...
        while(1)
        {
         //   printf("waiting for client...\n");
            ...
            tcp_pack.header=0xf8f8;
            tcp_pack.instruction=0x02;
             //tcp_pack.device_info=(car_device_info*)(recBuf+12);
             memcpy(recBuf,&tcp_pack,headerSize);
             memcpy(recBuf+headerSize,tcp_pack.device_info,tcp_pack.device_num*sizeof(car_device_info));
            if(-1==send(tcp_str->cli_sockfd,recBuf,tcp_pack.device_num*sizeof(car_device_info)+headerSize,MSG_NOSIGNAL)){
                goto LABEL_ACCEP;
            }
            usleep(100000);
            // car_process(tcp_pack);
...
void *tcp_ser_cam_thread(void *arg){
 ...
 while(1){
    ...
    addlen=sizeof(struct sockaddr);
    while(1){
  LABEL_ACCEP:tcp_str->cli_sockfd=accept(tcp_str->ser_sockfd,(struct sockaddr *)&(tcp_str->cli_addr),(socklen_t*)&addlen);
   ...
   if(-1==send(tcp_str->cli_sockfd,&packsize,4,MSG_NOSIGNAL)){
       close(tcp_str->cli_sockfd);
       goto LABEL_ACCEP;
   }
    for(i=1;i<n;i++)
    {
        ...
        if(-1==send(tcp_str->cli_sockfd,pt,PIC_PACK_SIZE,MSG_NOSIGNAL)){
            close(tcp_str->cli_sockfd);
            goto LABEL_ACCEP;
        }
        pt+=PIC_PACK_SIZE;
    ...
    int sendsiz_tmp=send(tcp_str->cli_sockfd,pt,buf.size()%PIC_PACK_SIZE,MSG_NOSIGNAL);
    if(-1==sendsiz_tmp){
        close(tcp_str->cli_sockfd);
        goto LABEL_ACCEP;
    }else{
